...
// Fungsi untuk memicu download PDF (Memanggil API Route Node.js)
const triggerDownload = (tilangId) => {
    try {
        const downloadUrl = `/api/bukti/${tilangId}`;
        const link = document.createElement('a');
        link.href = downloadUrl;
        link.download = `Bukti_Pembayaran_ETLE_${tilangId}.pdf`;
...
export default function Step5({ violationData, handleSuccessfulPayment, brivaNumber, tilangId }) {
    ...
    // --- TAMPILAN JIKA PEMBAYARAN SUDAH LUNAS (Pasca-Klik Tombol) ---
    if (isPaymentConfirmed) {
        return (
            <div className="text-center py-8 text-gray-900">
                ...
                {downloadError && (
                    <p className="text-sm text-red-600 mb-4 bg-red-50 p-3 rounded-md border border-red-300">
                        {downloadError}
                    </p>
                )}
                ...
                {/* Tombol Unduh Manual */}
                <button
                    onClick={() => triggerDownload(tilangId)}
                    className="mt-4 flex items-center justify-center gap-2 w-full max-w-xs mx-auto bg-red-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-red-700 disabled:bg-red-300"
                >
                    <FaDownload />
                    <span>Unduh Ulang Bukti Pembayaran</span>
                </button>

                <Link href="/" className="mt-4 flex items-center justify-center gap-2 w-full max-w-xs mx-auto bg-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-700">
    ...
    // --- TAMPILAN AWAL STEP 5 (INTRUKSI PEMBAYARAN) ---
    return (
        <div className="text-center text-gray-900">
            ...
            <div className="max-w-md mx-auto p-6 border rounded-lg bg-white">
                ...
                <p className="text-3xl font-bold text-blue-600 my-4 tracking-widest bg-gray-50 py-3 rounded-lg">{brivaNumber}</p>
            ...
            {/* Tombol Konfirmasi Setelah Pembayaran */}
            <button
                onClick={onConfirmPayment}
                disabled={isProcessing}
                className="mt-8 flex items-center justify-center gap-3 w-full max-w-xs mx-auto bg-green-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-green-700 disabled:bg-green-300"
            >
                {isProcessing ? (
                    <><FaSpinner className="animate-spin" /><span>Memproses Konfirmasi...</span></>
                ) : (
                    <span>Selesai & Konfirmasi Pembayaran</span>
                )}
            </button>
        </div>
    );
